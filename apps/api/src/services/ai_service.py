...
from core.errors import GenesisError, ErrorCode
    ...
    try:
        ...
        if response_schema:
            try:
                ...
                raise AIServiceError("LLM returned malformed JSON", details={"error": str(e)})
            except Exception as e:
                logger.error("Failed to validate LLM output against schema", extra={"error": str(e)})
        ...
        raise AIServiceError(f"Ollama API error: {e.error}")
    except Exception as e:
