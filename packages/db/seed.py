...
from genesis_db.governance import ModelRegistry, ModelProvider, PromptRegistry, PromptStatus, AgentRegistry, AgentRiskLevel
...
def seed_db():
    with Session(engine) as session:
        ...
        existing_model = session.exec(select(ModelRegistry).where(ModelRegistry.model_id == 'llama3.2:3b')).first()
        ...
        existing_prompt = session.exec(select(PromptRegistry).where(PromptRegistry.prompt_key == 'extract_requirements')).first()
  ...
  "extracted_requirements": [
    {
      ...
      "priority": "high|medium|low"
        ...
        existing_agent = session.exec(select(AgentRegistry).where(AgentRegistry.agent_key == 'requirement_agent')).first()
