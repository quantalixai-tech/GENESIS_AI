...
export function registerDevCommand(program: Command): void {
  program
    ...
    .action(async (opts: { api: boolean; web: boolean; portApi: string; portWeb: string }) => {
      ...
      if (!webOnly) {
        const pgPort = parseInt(dotenv['POSTGRES_PORT'] ?? '5432', 10);
        const pgOk = await isPortListening(pgPort);
        if (!pgOk) {
          logErr(`PostgreSQL is not reachable on port ${pgPort}.`);
          logInfo(`Run ${chalk.cyan('genesis start')} to start core infrastructure first.`);
          process.exit(1);
        }
        logOk(`PostgreSQL reachable on :${pgPort}`);
      }

      // Build DATABASE_URL from env
      const pgUser = dotenv['POSTGRES_USER'] ?? 'genesis';
      const pgPass = dotenv['POSTGRES_PASSWORD'] ?? 'genesis';
      const pgHost = dotenv['GENESIS_DEV_SERVICE_HOST'] ?? 'localhost';
      const pgPort2 = dotenv['POSTGRES_PORT'] ?? '5432';
      const pgDb = dotenv['POSTGRES_DB'] ?? 'genesis';
      const databaseUrl = `postgresql://${pgUser}:${pgPass}@${pgHost}:${pgPort2}/${pgDb}`;
          ...
          try {
            const proc = execa(
              ...
              [
                ...
                '--port', opts.portApi,
                '--reload-dir', apiSrc,
                '--reload-dir', path.join(pkgDb, 'genesis_db'),
                '--reload',
              ],
              {
                cwd: REPO_ROOT,
            ...
            const isExeca = typeof err === 'object' && err !== null && 'signal' in err;
            if (isExeca && (err as { signal?: string }).signal === 'SIGINT') return;
            logErr(`API process exited: ${err instanceof Error ? err.message : String(err)}`);
          }
        })();
      ...
      if (!apiOnly) {
        ...
        const webProc = (async () => {
          try {
            const proc = execa(
              'pnpm',
              ['--filter', 'web', 'dev', '--', '--port', opts.portWeb],
              {
                cwd: REPO_ROOT,
                env: {
                  ...mergedEnv,
            ...
            const isExeca = typeof err === 'object' && err !== null && 'signal' in err;
            if (isExeca && (err as { signal?: string }).signal === 'SIGINT') return;
            logErr(`Web process exited: ${err instanceof Error ? err.message : String(err)}`);
          }
        })();
      ...
      console.log('');
      if (both || !webOnly) console.log(`  ${c.info('API')}  → http://localhost:${opts.portApi}/docs`);
      if (both || !apiOnly) console.log(`  ${c.ok('Web')}  → http://localhost:${opts.portWeb}`);
      console.log('');
